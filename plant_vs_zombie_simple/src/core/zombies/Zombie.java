...
// ----------------------------------------
   ...
   public ArrayList<BufferedImage> frames;
    ...
    public void handleState() {
        if (this.state==Constants.WALK)this.walking();
        if (this.state==Constants.ATTACK)this.attacking();
        if (this.state==Constants.DIE)this.dying();
        if (this.state==Constants.FREEZE)this.freezing();
    }
    public void walking() {
        if (this.health <= 0) {
            this.setDie();
